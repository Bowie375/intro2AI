...
def cornersHeuristic(state, problem):
    ...
    for corner in Visited_Corners:
        tmp_visited_corners.append(corner)
    for corner in corners:
        if corner not in Visited_Corners:
            free_corners.append(corner)
    while len(free_corners)!=0:
        tmp_d=99999
        tmp_x=0
        tmp_y=0
        for corner in free_corners:
            x1,y1=corner
            tmp_d1=abs(x1-node[0])+abs(y1-node[1])
            if tmp_d1<tmp_d:
                tmp_d=tmp_d1
                tmp_x=x1
                tmp_y=y1
        node=(tmp_x,tmp_y)
        h_sum+=tmp_d
        tmp_visited_corners.append(node)
        free_corners=[]
        for corner in corners:
            if corner not in tmp_visited_corners:
                free_corners.append(corner)
        #a=tmp.index(min(temp,key=distance))
