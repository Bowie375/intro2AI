...
class InfoPane:
    def __init__(self, layout, gridSize):
        ...
        self.fontSize = min(20, layout.width * 3 // 2)
        self.textColor = PACMAN_COLOR
        self.predColor = GHOST_COLORS[0]
        self.drawPane(layout, gridSize)
        # self.image = []
    ...
    def drawPane(self, layout, gridSize):
        self.scoreText = text(self.toScreen(0, 0), self.textColor,
                              "SCORE:    0", "Times", self.fontSize, "bold")
        self.predictText = text(self.toScreen((layout.width) * gridSize - 10 * self.fontSize, 0), self.textColor,
                                "PREDICT: 0", "Times", self.fontSize, "bold")
    ...
    def updatePredict(self, predict):
        # print(predict)
        changeText(self.predictText, "PREDICT: % 4d" % predict)
...
class PacmanGraphics:
    ...
    def drawStaticObjects(self, state):
        ...
        self.drawFood(layout.food, layout.figureidx, layout.predy)
    ...
    def update(self, newState):
        ...
        self.infoPane.updatePredict(newState.predict)
        ...
        if newState._foodEaten != None:
            self.removeFood(newState._foodEaten, self.foodImages, self.predImages)
    ...
    def drawFood(self, foodMatrix, figureidx, predy):
        for xNum, x in enumerate(foodMatrix):
            for yNum, cell in enumerate(x):
                if cell:  # There's food here
                    (screen_x, screen_y) = self.to_screen((xNum, yNum))
                    digitImages=[]
                    idx = figureidx[xNum, yNum]
                    if idx > 1:
                        idx = 1
                    img = tkinter.PhotoImage(file='./image/{}.png'.format(idx))
                    self.image.append(img)
                    digitImages.append(image([screen_x-DEFAULT_FOOD_SIZE//2, screen_y-DEFAULT_FOOD_SIZE//2], img))
                    self.foodImages[(xNum,yNum)] = digitImages
        # return foodImages, predImages
    ...
    def removeFood(self, cell, foodImages, predImages):
        x, y = cell
        try:
            for digit in foodImages[(x,y)]:
                remove_from_screen(digit)
        except:
            pass
        try:
            for digit in predImages[(x,y)]:
                remove_from_screen(digit)
        except:
            pass
